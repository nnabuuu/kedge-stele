...
- `src/types.ts` — schema source of truth. `Decision`, `Edge`, `Milestone`, `Session`, `CapturePayload`, plus the 0.0.7 `Tag` / `Tagging` / `TagProposal` / `TagPolicy` types. If you change `Status`, `EdgeKind`, `MilestoneStatus`, `SessionSource`, or any of the tag-related types, `src/schemas.ts` (Zod) AND the capture form in `web/app.js` (`viewNew` / `buildDecision`) must be updated in lockstep — three places, no exceptions.
...
- `src/store.ts` — SQLite-backed graph. Owns nine tables: `decisions`, `edges`, `affects` (the decision graph), `milestones` and `sessions` (0.0.6+ grouping), `tags`, `taggings`, `tag_proposals` and `config` (0.0.7+ tagging + per-project preferences). Answers entity-anchored queries (`decisionsAffecting`) without any ontology — it keeps its own reverse index. The 0.0.6 schema migration adds `decisions.session_id` lazily via `ALTER TABLE` in a try/catch on every connect — pre-0.0.6 databases upgrade silently on first open. Session dedup is enforced by `UNIQUE(source, source_sess_id)`. Tag-name dedup is `UNIQUE COLLATE NOCASE` so `Security` and `security` resolve to one row. 0.0.7+: `PRAGMA journal_mode = WAL` is set in the constructor; the new tag tables are created via `CREATE TABLE IF NOT EXISTS` so pre-0.0.7 DBs pick them up without a migration step.
...
- `src/templates/` — source-of-truth for everything `stele init` / `stele hooks install` writes into a project: `decision-command.md` (slash command body), `stele-capture-skill.md` (skill with semantic-activation frontmatter), `stele-stop-hook.sh` (bash regex detector — as of 0.0.7 it also shells out to `stele config get tag_policy` and `stele tags list --json` to inject active tags into the hook's `additionalContext`). Changing project-installed content means changing here.
- `src/mcp.ts` — stdio MCP server. Registers 17 tools as of 0.0.7: the original four (`decision_capture` / `decision_resume` / `decision_trace` / `decision_resolve`), three milestone tools (`milestone_list` / `milestone_open` / `milestone_close`), eight tag tools (`tag_propose` / `tag_apply` / `tag_confirm` / `tag_reject` / `tag_recolor` / `tag_rename` / `tag_archive` / `tag_restore`), and two config tools (`config_get` / `config_set`). Catches `SteleNotInitializedError` at startup and exits 1 with a stderr hint, so Claude Code surfaces the "run stele init" message instead of a silent crash.
