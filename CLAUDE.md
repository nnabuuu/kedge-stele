...
- `src/templates/` — source-of-truth for everything `stele init` / `stele hooks install` writes into a project: `decision-command.md` (the `/decision` slash command), `milestone-report-command.md` (the 0.1.0 `/milestone-report` flow), `resume-command.md` (the 0.1.0 `/resume` flow), `stele-capture-skill.md` (the skill with semantic-activation frontmatter), `stele-stop-hook.sh` (bash regex detector — shells out to `stele milestones list --state going --json`, `stele config get tag_policy`, and `stele tags list --json` to inject active context). Changing project-installed content means changing here. `hooks.ts` writes each command file only if missing (no overwriting user edits).
