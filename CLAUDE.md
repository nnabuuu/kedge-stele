...
- `src/mcp.ts` — stdio MCP server. Registers **23 tools** as of 0.5.0: four capture-path tools (`decision_capture` / `decision_resume` / `decision_trace` / `decision_resolve`), six feature tools (`feature_list` / `feature_open` / `feature_decisions` / `feature_set_summary` / `feature_report` / `feature_complete` — the last marks a feature done and hand-closes its open/deferred loops via `Store.markFeatureComplete`, stamping `Decision.closedManually`), three session tools (`session_start` / `session_end` / `resume_command` — see note below), eight tag tools (`tag_propose` / `tag_apply` / `tag_confirm` / `tag_reject` / `tag_recolor` / `tag_rename` / `tag_archive` / `tag_restore`), and two config tools (`config_get` / `config_set`). `decision_resolve` takes `relation` (not `kind`) and accepts `depends_on`. Catches `SteleNotInitializedError` at startup; prints the legacy-DB migration hint via `store.migratedFromLegacy`. **0.5.0 boot**: now resolves the CLI locale via `setDefaultLocale(localeFromEnv())` before opening the store, then refines from the project's `display_language` config — same two-step pattern `cli.ts` uses, so `SteleNotInitializedError` and any other agent-visible prose comes back in the user's language.

  **Note on session_start / session_end / resume_command**: the agent-facing capture surface largely doesn't use these anymore — sessions auto-bucket inside `decision_capture` and the resume concept folded into `/stele:feature`. They stay registered for legacy callers and for callers (CLI `stele sessions ...`, the SPA's resume launcher) that still hit the underlying mechanism. Consider retiring them in a future release once we're sure nothing important calls them.
...
**The `main_language` config (0.4.1).** Free-text per-project setting stored in the existing `config` table (no schema change). When set, the SessionStart hook injects a `主语言 / main language: <value>` block with a verbatim-terms rule, between `tag policy` and the standing capture criteria. The skill / both slash commands cross-reference: every free-text Decision field (`title`, `detail.context`, `detail.options[].rationale`, `detail.decision`, `detail.consequences`, `Feature.summary`) gets written in that language; technical terms, ids, file paths, code identifiers, and proper nouns stay verbatim. Unset → no inject → agent uses the conversation's language as before. The directive is intentionally bilingual in the hook output so the agent reads it whether the rest of the context block is CN or EN.
