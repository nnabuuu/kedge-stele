...
0.3.0 collapsed the three 0.2-era commands (`/decision`, `/milestone-report`, `/resume`) into **one**:

- `.claude/commands/stele/feature.md` — `/stele:feature`. Idempotent reconcile pass, callable any time. Finds the project's currently-going Feature (or opens one), pulls every captured decision on it via `feature_decisions`, diffs against the live transcript to identify gaps, captures each gap via `decision_capture`, then rewrites the rolling `Feature.summary` via `feature_set_summary` (replace, not append). No session-end ritual, no pause reason, no state transition — Feature state moves to `winding` / `done` manually via the dashboard.

The retired commands are not aliased; re-running `stele init` on an upgraded project deletes the legacy files. If you change the `Decision` schema in `types.ts`, update both the `stele-capture` skill (the field-level reference) and `stele-feature-command.md` (the workflow script) in lockstep.
...
The canonical spec is `design/Stele Backend Design.md`. **As of 0.3.0, the backend and the web SPA are both aligned with the spec.** What's still deferred:
