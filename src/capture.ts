// Feature + Feature + Session resolution for `decision_capture`. Lifted
// out of mcp.ts in 0.0.6-fix so it's directly unit-testable.
//
// 0.1.0 changes:
//   • Feature has a `featureId` FK; we resolve it from feature.draft.featureId
//     or .featureDraft, or fall back to the per-project "unscoped" Feature.
//   • Feature state starts at "going" when a Session is opened on it
//     (formerly "active").
//   • Adds `recordSessionStart`, `recordSessionEnd`, `recordFeatureReport`
//     helpers used by the new MCP tools `session_start` / `session_end` /
//     `feature_report`.
//   • `mode='unscoped'` now resolves to a real Feature (the auto-created
//     per-project unscoped feature), so every Decision has a feature_id
//     FK and the `<feature>/<local>` id format works uniformly.
import { createHash } from "node:crypto";
import type { Store } from "./store.ts";
import type {
  CaptureFeatureMode,
  CaptureSourceSession,
  Feature,
  FeatureId,
  PauseReason,
  Project,
  Session,
  SessionId,
  SessionOutcome,
  SessionProvenance,
  SessionSource,
} from "./types.ts";

export interface ResolvedFeatureSession {
  featureId: FeatureId;
  sessionId: SessionId;
  notes: string[];
}

export function newSessionId(source: string, sourceSessionId: string | undefined): string {
  // The UNIQUE(source, source_sess_id) constraint in the store is the real
  // dedup guarantee; the local id just needs to be primary-key-unique.
  const seed = `${source}|${sourceSessionId ?? Math.random()}|${Date.now()}`;
  const hash = createHash("sha256").update(seed).digest("hex").slice(0, 8);
  return `ses-${hash}`;
}

/**
 * Guard: every operation that creates features / features / sessions
 * needs to know which Project owns them. Each store has at most one Project
 * (the per-project DB invariant); this helper surfaces a clear error when
 * the Project row is missing (usually means `stele init` hasn't been run).
 */
function requireProject(store: Store): Project {
  const p = store.theProject();
  if (!p) {
    throw new Error(
      "no Project row in this store — run `stele init` first (or call ensureProject)",
    );
  }
  return p;
}

/**
 * Wires (feature, sourceSession) into actual rows on the Store. Returns
 * the feature_id + session_id to stamp on the decision.
 *
 * Behaviour for `mode: "continue"`:
 *   - if the matching Session is on a *different* feature than requested,
 *     the Session is reassigned to the requested feature (the agent's
 *     latest judgment wins; older Decisions on that session move with it).
 *   - if the matching Session is on the same feature, it's reused.
 *
 * Behaviour for `mode: "new"`:
 *   - a fresh Feature is created with state='draft'.
 *   - then session resolution proceeds as above.
 *
 * Behaviour for `mode: "unscoped"` (or undefined):
 *   - resolves to the auto-created unscoped Feature; a Session is still
 *     opened so the resume strip can show the latest activity.
 */
export function resolveFeatureAndSession(
  store: Store,
  feature: CaptureFeatureMode | undefined,
  sourceSession: CaptureSourceSession | undefined,
  decisionAt: string,
): ResolvedFeatureSession {
  const notes: string[] = [];
  const project = requireProject(store);

  // 1. Resolve the feature
  let featureId: FeatureId;
  if (!feature || feature.mode === "unscoped") {
    const m = store.ensureUnscopedFeature(project.id);
    featureId = m.id;
    notes.push(`bound to unscoped feature ${m.id}`);
  } else if (feature.mode === "continue") {
    const existing = store.getFeature(feature.id);
    if (!existing) throw new Error(`feature "${feature.id}" does not exist`);
    featureId = existing.id;
    notes.push(`continued feature ${existing.id} "${existing.name}"`);
  } else {
    // mode: "new"
    const id = store.nextFeatureId();
    const m: Feature = {
      id,
      projectId: project.id,
      name: feature.draft.name,
      state: "draft",
      about: feature.draft.about,
      startedAt: decisionAt,
    };
    store.putFeature(m);
    featureId = id;
    notes.push(`opened feature ${id} "${m.name}"`);
  }

  // 2. Resolve (or create / reassign) the Session
  if (!sourceSession) {
    // No source identity — open an anonymous "manual" session.
    const id = newSessionId("manual", undefined);
    store.putSession({
      id, featureId, source: "manual", startedAt: decisionAt,
    });
    notes.push(`opened anonymous session ${id}`);
    advanceFeatureFromDraft(store, featureId);
    return { featureId, sessionId: id, notes };
  }

  if (sourceSession.sourceSessionId) {
    const existing = store.findSession(sourceSession.source, sourceSession.sourceSessionId);
    if (existing) {
      if (existing.featureId !== featureId) {
        notes.push(
          `reassigned session ${existing.id} from ${existing.featureId} → ${featureId} (older decisions on this session move with it)`,
        );
        store.putSession({ ...existing, featureId });
      } else {
        notes.push(`reused session ${existing.id}`);
      }
      advanceFeatureFromDraft(store, featureId);
      return { featureId, sessionId: existing.id, notes };
    }
  }

  // New Session
  const id = newSessionId(sourceSession.source, sourceSession.sourceSessionId);
  store.putSession({
    id,
    featureId,
    source: sourceSession.source,
    sourceSessionId: sourceSession.sourceSessionId,
    startedAt: decisionAt,
  });
  notes.push(`opened session ${id} (${sourceSession.source})`);
  advanceFeatureFromDraft(store, featureId);
  return { featureId, sessionId: id, notes };
}

/**
 * When a session opens on a feature in 'draft' state, advance it to 'going'.
 * Other state transitions are explicit (`session_end` may move to 'winding'
 * if outcome.type='resolved'; `feature_report` may suggest others).
 */
function advanceFeatureFromDraft(store: Store, featureId: FeatureId): void {
  const m = store.getFeature(featureId);
  if (m && m.state === "draft") store.setFeatureState(featureId, "going");
}

// ===========================================================================
// 0.1.0 — explicit session_start / session_end helpers
// ===========================================================================

/**
 * Explicitly open a Session under a feature. Used by the `session_start`
 * MCP tool. Idempotent on (source, sourceSessionId): if a Session already
 * exists for the (source, sourceSessionId) pair, returns it (and reassigns
 * to the new feature if needed).
 */
export function recordSessionStart(
  store: Store,
  featureId: FeatureId,
  sourceSession: CaptureSourceSession,
  provenance?: SessionProvenance,
  at?: string,
): Session {
  if (!store.getFeature(featureId)) {
    throw new Error(`feature "${featureId}" does not exist`);
  }
  const startedAt = at ?? new Date().toISOString();

  if (sourceSession.sourceSessionId) {
    const existing = store.findSession(sourceSession.source, sourceSession.sourceSessionId);
    if (existing) {
      // If the prior session was already closed, we're starting a NEW logical
      // session with the same (source, sourceSessionId). Open a fresh row —
      // don't silently revive the ended one (that leaves endedAt + outcome
      // pointing at the wrong moment in time).
      if (existing.endedAt) {
        // The UNIQUE(source, source_sess_id) constraint forces us to clear
        // the old row's source_sess_id before inserting the new row. Mark it
        // as `null` and rewrite — auditors can still find it via id.
        store.putSession({ ...existing, sourceSessionId: undefined });
      } else {
        const next: Session = {
          ...existing,
          featureId,
          provenance: provenance ?? existing.provenance,
        };
        store.putSession(next);
        advanceFeatureFromDraft(store, featureId);
        return next;
      }
    }
  }
  const id = newSessionId(sourceSession.source, sourceSession.sourceSessionId);
  const s: Session = {
    id,
    featureId,
    source: sourceSession.source,
    sourceSessionId: sourceSession.sourceSessionId,
    startedAt,
    provenance,
  };
  store.putSession(s);
  advanceFeatureFromDraft(store, featureId);
  return s;
}

/**
 * Close a Session with an outcome and an optional pause_reason. When
 * outcome.type='resolved', the feature advances to 'winding' (if it was
 * 'going'). Other state changes stay explicit (use feature_report's
 * nextStateSuggestion to nudge a richer transition).
 */
export function recordSessionEnd(
  store: Store,
  sessionId: SessionId,
  outcome: SessionOutcome,
  pauseReason?: PauseReason,
  at?: string,
): Session {
  const s = store.getSession(sessionId);
  if (!s) throw new Error(`session "${sessionId}" does not exist`);
  const endedAt = at ?? new Date().toISOString();
  const next: Session = { ...s, endedAt, outcome, pauseReason };
  store.putSession(next);

  // outcome.resolves[] + outcome.via materialise as real `resolves` edges so
  // resumeDigest stops surfacing the closed loops next time. Without this
  // the typed-resolved outcome is cosmetic only.
  if (outcome.type === "resolved" && outcome.resolves && outcome.via) {
    for (const closedId of outcome.resolves) {
      // addEdge flips target status='resolved' + writes resolved_by.
      store.addEdge({ from: outcome.via, to: closedId, relation: "resolves" });
    }
  }

  if (outcome.type === "resolved") {
    const m = store.getFeature(s.featureId);
    if (m && m.state === "going") store.setFeatureState(m.id, "winding");
  }
  return next;
}
